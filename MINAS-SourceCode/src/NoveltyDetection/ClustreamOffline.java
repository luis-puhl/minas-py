...
public class ClustreamOffline {
    ...
    private int clusterExample[]; // vector of size: number of examples. Each position contains the number of the
                                  // cluster in which the example was classified
     ...
     ******************** CluStreamOff *************************************************
     ********************************************************************************/
    public ArrayList<double[]> CluStreamOff(String classLabel, String filepath, float nMicro, boolean flagMicro)
            throws IOException {
        ...
        // ********************if the microclusters have to be macroclustered using the
        // KMeans
        if (flagMicro == false) {
        ...
        // else: using the micro-clusters without macro-clustering
            ...
            int value;
            // to keep a vector of size: number of elements. Each position contains the
            // number of the cluster of the corresponding element
            for (int i = 0; i < elemMicroCluster.size(); i++) {
    ...
    // in the clustream it is not possible to obtain this, but it is possible to
    // obtain the radius that is the mean distance between elements and centroid *2
    public double[] getRadius() {
        double[] radius = new double[numResultingClusters];
        meanDistance = new double[numResultingClusters];

        if (flagMicro == false) { // in this case it was used clustream + kmeans (macro-clustering in the
                                  // micro-clusters)
            double[] centeres;
            double maxDistance;
            double[] elem;
