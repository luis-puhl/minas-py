...
public class Evaluation {
    ...
    public void addExampleConfusionMatrix(String strExClassification) throws IOException {
        ...
        // obtaining the true class and minasclass from the string containing the
        // example classification
        String[] data = strExClassification.split("\t");
        TrueClass = "C" + data[1].split(":")[1];
        MINASClass = data[2].split(":")[1].trim();

        // examples classified as extension of a class will computed as belonging to the
        // class
        if (MINASClass.split(" ")[0].equals("ExtCon")) {
            MINASClass = MINASClass.replace("ExtCon", "C");
        }
        // examples classified as extension of a novelty pattern will computed as
        // belongint to the novelty pattern
        else if (MINASClass.split(" ")[0].equals("ExtNov")) {
            MINASClass = MINASClass.replace("ExtNov", "N");
        }
        // if the true class of the example is a novel class, then add a line in the
    ...
    public void updateConfusionMatrix(String info, Hashtable confusionMatrixTmp) {
        ...
        // updating the confusion matrix, in order to remove the examples from the
        // unknown column and put in the corresponding novelty
        ...
        for (int i = 0; i < problemClassesName.size(); i++) {
            if (confusionMatrixTmp.get(problemClassesName.get(i)) != null) {
                ...
                unkValue = Integer
                        .parseInt(((Hashtable) confusionMatrix.get(problemClassesName.get(i))).get("Unk").toString());
                ...
                oldValue = Integer.parseInt(
                        ((Hashtable) confusionMatrix.get(problemClassesName.get(i))).get(classLabel).toString());
    ...
    public int printResultsGraph(String filepath) throws IOException {
        ...
        for (int i = 0; i < vectorExtension.size(); i++) {
            if (i > 0) {
                if (Integer.parseInt(vectorExtension.get(i) + "") != Integer
                        .parseInt(vectorExtension.get(i - 1) + "")) {
        ...
        // generating a graph (using FreeChart) in order to show the evaluation measures
        // over the stream
        FreeChartGraph t = new FreeChartGraph();
        t.createGraph(vectorMeasure1, vectorMeasure2, vectorMeasure3, vectorTimestamp, vectorNovelty, vectorExtension,
                flgEvaluationType);
    ...
    public Hashtable calculateConfusionMatrixPerc() {
        ...
        for (int i = 0; i < problemClassesName.size(); i++) {
            lineSum = 0;
            for (int j = 0; j < columnName.size(); j++) {
                lineSum += Integer.parseInt(
                        ((Hashtable) confusionMatrix.get(problemClassesName.get(i))).get(columnName.get(j)).toString());
            }
            for (int j = 0; j < columnName.size(); j++) {
                cellValue = Integer.parseInt(
                        ((Hashtable) confusionMatrix.get(problemClassesName.get(i))).get(columnName.get(j)).toString());
                ((Hashtable) confusionMatrixPct.get(problemClassesName.get(i))).put(columnName.get(j),
                        (double) cellValue / lineSum);
            }
        }
    ...
    // ***************************************************************************************************
    /*
     * public int ExibeResultadosFolds(String nomeArquivo) throws IOException{ int
     * nroLinhas = ((Hashtable)MatrizesConfusaoPorcFolds.get(0)).size(); ArrayList
     * <String> nomeColuna = new ArrayList<String>();
     * 
     * //cada fold pode produzir tabelas com diferente nro de colunas (algumas folds
     * encontram +/- novidades q outras) // encontrar o maior nro de colunas int
     * nroColunas = 0; for (int i=0; i< MatrizesConfusaoPorcFolds.size(); i++){ if
     * (((Hashtable)((Hashtable)MatrizesConfusaoPorcFolds.get(i)).get(
     * problemClassesName.get(0))).size() > nroColunas) nroColunas =
     * ((Hashtable)((Hashtable)MatrizesConfusaoPorcFolds.get(i)).get(
     * problemClassesName.get(0))).size(); }
     * 
     * double soma; FileWriter arquivoSaida = new FileWriter(nomeArquivo);
     * 
     * //impressao dos titulos for (int j=0; j<offlineClassesName.size(); j++){
     * arquivoSaida.write("," + offlineClassesName.get(j));
     * nomeColuna.add(offlineClassesName.get(j)); } int nroNov = nroColunas -
     * offlineClassesName.size() -1; for (int j=0; j<nroNov; j++){
     * arquivoSaida.write("," + "N " + (j+1)); nomeColuna.add("N " + (j+1)); }
     * arquivoSaida.write ("," + "nao sei"); nomeColuna.add("n�o sei");
     * 
     * //calculo da media das n-folds for (int i=0; i< nroLinhas; i++){
     * arquivoSaida.write("\n" + problemClassesName.get(i)); for (int j=0;
     * j<nroColunas; j++){ soma = 0; for (int k=0;
     * k<MatrizesConfusaoPorcFolds.size(); k++){ if
     * (((Hashtable)((Hashtable)MatrizesConfusaoPorcFolds.get(k)).get(
     * problemClassesName.get(i))).containsKey(nomeColuna.get(j))) soma = soma +
     * Double.parseDouble(((Hashtable)((Hashtable)MatrizesConfusaoPorcFolds.get(k)).
     * get(problemClassesName.get(i))).get(nomeColuna.get(j)).toString()); else
     * soma+=0; } arquivoSaida.write("," +
     * (double)soma/MatrizesConfusaoPorcFolds.size()); } } arquivoSaida.close();
     * return 0; }
     */
    ...
    public float[] computeMeasuresProposedMethod() {
        ...
        // computing FMeasure
        float resFMeasure[] = computeFMeasure(numTruePositivePerClass, numFalsePositivePerClass,
                numFalseNegativePerClass);
        ...
        for (int e = 0; e < numTrueNegativePerClass.length; e++) {
            numTrueNegativePerClass[e] = numTotalExampleNoUnk
                    - (numTruePositivePerClass[e] + numFalseNegativePerClass[e] + numFalsePositivePerClass[e]);
        }
        ...
        for (int e = 0; e < combinedErrPerClass.length; e++) {
            if ((numFalsePositivePerClass[e] + numTrueNegativePerClass[e]) != 0)
                falsePositiveRate = (float) numFalsePositivePerClass[e]
                        / (numFalsePositivePerClass[e] + numTrueNegativePerClass[e]);
            else
                falsePositiveRate = 0;
            if ((numFalseNegativePerClass[e] + numTruePositivePerClass[e]) != 0)
                falseNegativeRate = (float) numFalseNegativePerClass[e]
                        / (numFalseNegativePerClass[e] + numTruePositivePerClass[e]);
            else
                falseNegativeRate = 0;
        ...
        if (numTotalExampleNoUnk == 0)
            ...
            combinedErr = sumCombinedErr / (2 * numTotalExampleNoUnk);
    ...
    public float[] computeMeasureLiteratureMethod() {
        ...
        for (i = 0; i < offlineClassesName.size(); i++) {
            for (int j = 0; j < ((Hashtable) confusionMatrix.get(offlineClassesName.get(i))).size(); j++) {
                cell = Integer.parseInt(
                        ((Hashtable) confusionMatrix.get(offlineClassesName.get(i))).get(columnName.get(j)).toString());
        ...
        for (int k = i; k < nroClassesProb; k++) {
            for (int j = 0; j < columnName.size(); j++) {
                cell = Integer.parseInt(
                        ((Hashtable) confusionMatrix.get(problemClassesName.get(k))).get(columnName.get(j)).toString());
        ...
        if (NroExCNormal == 0)
            ...
            FNew = (float) (Fp * 100) / NroExCNormal;
    ...
    public ArrayList<Integer>[] computeCorrespondeProblemClassNovMINAS() {
        ...
        // for each novelty, to find the corresponding class, based on the number of
        // classified examples
        for (int i = offlineClassesName.size(); i < columnName.size() - 1; i++) {
            ...
            for (int j = 0; j < problemClassesName.size(); j++) {
                cell = Integer.parseInt(
                        ((Hashtable) confusionMatrix.get(problemClassesName.get(j))).get(columnName.get(i)).toString());
                if (cell > max) {
                    max = cell;
                    maxPos = j;
                }
    ...
    // *******************************************************
    // **********************************************************************************************
    public void computeFPFN(ArrayList<Integer>[] vetorCorrespondenceCProbNovMINAS, int numTruePositivePerClass[],
            int numFalsePositivePerClass[], int numFalseNegativePerClass[]) {
        int cell, pos;
    ...
    // ***********************************************
    // **********************************************************************************************
    public float[] computeFMeasure(int numTruePositivePerClass[], int numFalsePositivePerClass[],
            int numFalseNegativePerClass[]) {
        float pi, ro;
        ...
        for (i = 0; i < problemClassesName.size(); i++) {
            if (numTruePositivePerClass[i] + numFalseNegativePerClass[i] > 0) {
                if (numTruePositivePerClass[i] + numFalsePositivePerClass[i] == 0)
                    ...
                    pi = (float) numTruePositivePerClass[i]
                            / (numTruePositivePerClass[i] + numFalsePositivePerClass[i]);
                ro = (float) numTruePositivePerClass[i] / (numTruePositivePerClass[i] + numFalseNegativePerClass[i]);
                if ((pi + ro) == 0)
                    FMeasurePorClasse[i] = 0.0f;
                else
                    FMeasurePorClasse[i] = (float) (2 * pi * ro) / (pi + ro);
