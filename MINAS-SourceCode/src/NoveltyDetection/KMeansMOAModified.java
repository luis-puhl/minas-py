...
public class KMeansMOAModified {
	...
	public Clustering kMeans2(Cluster[] centers, List<? extends Cluster> data,double soma[], ArrayList<Integer> clusterSize, int elemCluster[] , ArrayList<Double> maxDistance, ArrayList<Double> meanDistance) {
        ...
        //keeping a vector that contains in each position, the number of elements of the clusters
    ...
    public void getRadiusMeanDistance(ArrayList<ArrayList<Cluster>> clustering, Clustering centers, ArrayList<Double> maxDistance, ArrayList<Double> meanDistance){
        int k = centers.size();             
		double [] centro;
		double maiorDist;
		double [] elem;
...
******************** OMRk ******************************************************
********************************************************************************/    
    public static Clustering OMRk(Clustering microClusteringResult, int algCentroidChoice, ArrayList<ArrayList<Integer>>elemMacroCluster) {
    		...
    		//System.out.print("\n K:" + valorK + "  ");
    	 	for (int i=0; i<10; i++){
    	 		switch (algCentroidChoice){    	 
	    	 		case 0: centers = sequentialCenters(microclusters, k);break;
    	 		}
...
******************** calculateSilhouette ***************************************
********************************************************************************/ 
    public static double calculateSilhouette(Clustering clusters, ArrayList<CFCluster> microclusters,ArrayList<ArrayList<Integer>>elemMacroCluster ){
	    ...
	    for (int p=0; p<clusterElem.length;p++){
	    	Integer ownClusters=clusterElem[p];
	       	...
	       	double distancePointToCluster = distance(microclusters.get(p).getCenter(),clusters.get(ownClusters).getCenter());
	       	double distancePointToCloserCluster = distanceByClusters[0];
	
	       	pointSilhouette += (distancePointToCloserCluster - distancePointToCluster)/Math.max(distancePointToCloserCluster, distancePointToCluster);
	    }
	    ...
	    for ( int i = 0; i < k; i++ ) {
	    	...
	    	if (clustering.get(i).size() == 0){ // cluster do not have element
	    		...
	    		for (int n =0; n< clustering.get(posMax1).size();n++){
	    			double distance1 = distance(clustering.get(posMax1).get(n).getCenter(),centers[posMax1].getCenter());
	    			double distance2 = distance(clustering.get(posMax1).get(n).getCenter(),clustering.get(i).get(0).getCenter());
