...
public class MINAS {
    ...
    ArrayList<Cluster> SleepModeMem;		      //	 temporary memory where are stored the clusters that does not receive examples for a long time
    ...
    private int flagEvaluationType;				  //     evaluation type: 0 -- proposed metodology, 1 -- methodology from the literature
...
******************** Constructor *********************************************
********************************************************************************/    
    public MINAS(String fileOff, String fileOnl, String outDirec,String algOff, String algOnl, double threshold, int evaluationType, int thresholdForgetPast, int nMicro, boolean flag) throws IOException {
    	this.filenameOffline = fileOff;
    	this.filenameOnline = fileOnl;
        this.algClusteringOff = algOff;
        this.algClusteringOnl = algOnl;
        this.outputDirectory = outDirec + "\\out_MINAS\\" + algOff + "\\" + algOnl+"_minexcl" + minExCluster + "_valcrit" + validationCriterion + "_algOff" + algOff + "_t" + threshold + "_f" + thresholdForgetPast;
...
********************************************************************************/       
    public void validationcriterion() {
        ...
        //if (validationCriterion.compareTo("wss") != 0 && validationCriterion.compareTo("dec") != 0 && validationCriterion.compareTo("vol") != 0) {
...
******************** classify  *************************************************
*********************************************************************************/
	public int classify(Evaluation av, int count_k,String[]  dataEx, String textFileOut, int countEx, ArrayList<Double> vectorMeasuresModelOffline, ArrayList<String> Novelties) throws IOException{       
...
//********** example identify belonging to one of the clusters that composes the currrent Model
        	...
        	// if the number of examples in dataUnk exceed a limit, the remove the oldest one
        	/*if(dataUnk.size() > maxExUnk ) {
        	     dataUnk.remove(0);
        	 }*/
        	            
        	//********** Searching for new clusters ****************************
        	//if there are a sufficient number of examples to execute a clustering procedure
        	//and the last check had been executed at least (count_k*minExCluster) time units earlier  
        	//Se o numero de exemplos eh suficiente (eh possivel gerar grupos com, em media, o numero minimo de exemplos)
        	count_k = numMicro;
        	if((dataUnk.size() >= count_k * minExCluster) && (lastCheck + (count_k * minExCluster) < countEx)) {
	        	...
	        	//creating temporary model (set of clusters) from unknown data --> clustering procedure
	        	...
	        	//temporary confusion matrix 
	        	 ...
	        	 for(int cluster = 0; cluster<tmpModelUnk.size(); cluster++) {
	        	     ...
	        	     if(clusterUnk.getSize()>=minExCluster){ 
	        	    	 ...
	        	    	 //the current cluster has the meanDistExCen <= maximum meanDistExCen from the offline model * 2
	        	         if( (( validationCriterion.equalsIgnoreCase("dec") ) && ( clusterUnk.getMeanDistance() <= max(vectorMeasuresModelOffline) *2 ))){      
	        	        	 flgTodosInvalidos = 0;                  
	        	//********** The cluster is valid ****************
	        	            ...
	        	            for(int elem = 0; elem < tmpRemove.length; elem ++){        	            	
	        	            	...
	        	            	if(tmpRemove[elem] == cluster){ //if the cluster of the unknown example == to the i_th valid cluster
	        	            		...
	        	            		if ((confusionMatrixTmp.get("C " + dataUnk.get(elem)[size-1]))== null){
	        		            		...
	        		            		int valor = ((Integer)confusionMatrixTmp.get("C " + dataUnk.get(elem)[size-1])).intValue();
	        		            		confusionMatrixTmp.put("C " + dataUnk.get(elem)[size-1], new Integer(valor+1));	        	            		 
	        		            	}
	        	            	}	        	            	
	        	            }   
	        	            ...
	        	            // to find the closest cluster to the new valid cluster 
	        			     ...
	        			     String text="";
	        			     //if the distance (Euclidean) between the centroid of the new valid cluster and its closest cluster is less than a threshold
	        			     if(distance < thresholdValue){//then: the new valid cluster in an extension of a concept
	        			         ...
	        			         if((category.equalsIgnoreCase("normal")) || (category.equalsIgnoreCase("ext"))){
	        			             ...
	        			             text = "Thinking "+"Extension: "+ "C " + classLabel +" - "+(int)clusterUnk.getSize()+" examples";                    
	        			         }      
	        			         //extension of a novelty pattern learned online
	        			         else{	        			        	 
	        			             updateModel(clusterUnk, classLabel,"extNov");
	        			             text = "Thinking "+"NoveltyExtension: "+ "N " + classLabel +" - "+(int)tmpModelUnk.get(cluster).getSize()+" examples";
	        			         }      
	        			     }                    
	        			     else{
	        			    	 //else: the new valid cluster is a novelty pattern
	        			         updateModel(clusterUnk, Integer.toString(Novelties.size()+1),"nov"); 
	        			         //if the valid clusters is really identified as novelty (may be a novelty intercepte another novelty and thus it is considered an extension                      
	        			         if(model.get(model.size()-1).getLblClasse().compareToIgnoreCase(Integer.toString(Novelties.size()+1))== 0){
	        			             ...
	        			             if (verifyClassRecurrence(clusterUnk)){
	        			            	 if (clusterUnk.getCategory().compareToIgnoreCase("ext")==0)
	        			            		 text = "Thinking "+"Extension:"  +"C " + model.get(model.size()-1).getLblClasse() + " - "+(int)tmpModelUnk.get(cluster).getSize()+" examples";
	        			                 else
	        			                	 text = "Thinking "+"NoveltyExtension: " + "N "+ model.get(model.size()-1).getLblClasse() + " - "+(int)tmpModelUnk.get(cluster).getSize()+" examples";
	        			                 //System.out.println("Recurrence of the class" +(tmpModelUnk.get(cluster).getLblClasse())+" - "+(int)tmpModelUnk.get(cluster).getSize()+" examples");		                        		
	        			             }
	        			             else {
	        			            	 //the class label of a new valid cluster considered as a novelty is a sequential number
			                        	Novelties.add(Novelties.size()+1+"");
			                        	text = "ThinkingNov: "+"Novidade " +(Novelties.size() )+" - "+(int)tmpModelUnk.get(cluster).getSize()+" examples"; 
	        			             }
	        			         }
	        			         else{
	        			        	 text = "Thinking "+"NoveltyExtension: "+ "N " +model.get(model.size()-1).getLblClasse() +" - "+(int)tmpModelUnk.get(cluster).getSize()+" examples";	                        	
	        	         ...
	        	         //Se o grupo foi considerado invalido por ser muito esparso 
	        	        	 ...
	        	        	 //Aumentar o contador deste tipo de falha, para a configuracao automática de k
	        			     temp_count_toosparse ++;
	        	         }
	        	     } //Fim tem o numero minimo de exemplos  
	        	     // Se o grupo foi considerado inválido por nao ter o numero minimo de exemplos     
	        	     else{
	        	    	 //Aumentar o contador deste tipo de falha, para a configuração automática de k
	        	         temp_count_toofewex ++;
	        	     }                
	        	} //Fim para cada grupo candidato
	        	                                
	        	//********** Adaptacao Automatica de k **********
	        	count_k = adaptationK(flgTodosInvalidos, temp_count_toosparse, temp_count_toofewex, count_k, dataUnk.size());
...
********************************************************************************/     
    public void online(ArrayList<String> classesOffline, Evaluation av) throws IOException{   
        ...
        //obtaining information about the clusters obtained offline (used to verify if a new cluster is valid)
...
//***********fim da classificacao da instancia*****************
          ...
          //put the inative clusters (if they do not receive examples for x timestamps) in a sleepMemory 
...
******************** adaptationK ***********************************************
********************************************************************************/
    public int adaptationK(int flgTodosInvalidos, int temp_count_toosparse, int temp_count_toofewex, int count_k, int tamDataUnk){
    	if(flgTodosInvalidos == 1){
	    	...
	    	if(temp_count_toosparse > temp_count_toofewex){                    
	    		//Se k eh menor do que um limite superior para k, no qual cada grupo tem em media o numero minimo de exemplos
...
********************************************************************************/   
   public ArrayList<String> identClosestCluster(Cluster validCluster,double threshold ){
       ...
       //compute threshold: mean distance between the examples of the cluster to the centroid multiplied by a factor (user input)
...
********************************************************************************/    
   public ArrayList<Cluster> createTmpModelUnk(int k, int [] vectorResClustering) throws IOException {
       ...
       //in order to execute de clustream algorithm, the data needs to be stored in a file 
   ...
   ******************** createModelKMeans *****************************************
   ********************************************************************************/    
   public ArrayList<Cluster> createModelKMeans(String filename, int k, String cl, ArrayList<String []>data,int [] clusters) throws NumberFormatException, IOException{	        
       String line; 
       int cont=0;
       int lineSize=0;
       ...
       for (int w = 0; w < k; w++) {
       		Cluster clus = new Cluster(meanDistance.get(w),centers.get(w).getCenter(),clusterSize.get(w),radius.get(w),cl,"normal",timestamp);
       		clusterSet.add(clus);
       }   
       return clusterSet;
   }              
    ...
    ********************************************************************************/    
    public ArrayList<Cluster> createModelClustream(String filename, String cl,int [] vectorResClustering) throws NumberFormatException, IOException{
        ...
        for (int w = 0; w < size.length; w++) {
            Cluster clus = new Cluster(meanDistance[w],centers.get(w),size[w], radius[w], classLabel[w],"normal",timestamp);
            clusterSet.add(clus);
        }
        return clusterSet;
    }
	   ...
	   if (SleepModeMem.size() > 0){
		   ...
		   int pos = 0;
	        ...
	        if ((minDist*minDist) <= vthreshold){
	        	if((SleepModeMem.get(pos).getCategory().equalsIgnoreCase("normal")) || (SleepModeMem.get(pos).getCategory().equalsIgnoreCase("ext")))
...
********************************************************************************/
    public static void main(String[] args) throws IOException {        
    	//MINAS m = new MINAS("balancescale_nor1", "C:\\Users\\Elaine\\Documents\\", "clustream", 1.1, "data_s_n_f_s");
    	//MINAS m = new MINAS("iris_nor1", "C:\\Users\\Elaine\\Documents\\", "kmeans", 1.1, "data_s_n_f_s",10);
        //MINAS m = new MINAS("MOA3", "C:\\Users\\Elaine\\Documents\\", "clustream", "clustream", 2.0, "data_s_n_f_s",1,0);
    	//MINAS m = new MINAS("SynDMenor", "C:\\Users\\Elaine\\Documents\\", "kmeans", 2.0, "data_s_n_f_s",1);
    	//MINAS m = new MINAS("SynDMenor", "C:\\Users\\Elaine\\Documents\\", "clustream", 1.1, "data_s_n_f_s",1);
    	//MINAS m = new MINAS("KDDTe", "C:\\Users\\Elaine\\Documents\\", "clustream", "clustream", 3.0, "data_s_n_f_s",1);
    	//MINAS m = new MINAS("KDDTe5Classes", "D:\\TestesMinas\\", "clustream", "clustream",2.0, "data_s_n_f_s",1,1);
    	//MINAS m = new MINAS("fcTe", "C:\\Users\\Elaine\\Documents\\", "clustream", "clustream", 1.1, "data_s_n_f_s",1,1);
    	MINAS m = new MINAS("C:\\Users\\Elaine\\Documents\\data_s_n_f_s\\covtypeOrigNorm_off", "C:\\Users\\Elaine\\Documents\\data_s_n_f_s\\covtypeOrigNorm_onl","E:\\resultados","clustream", "clustream",2.0,1, 10000,100, true);
    	m.execution();
    }
}
